...
def get_activities(
    ...
    if difficulty:
        ...
        # (activities with no difficulty are available for all levels)
        # Use $and to safely combine with other existing filter conditions
        query.setdefault("$and", [])
        query["$and"].append(
            {"$or": [{"difficulty": difficulty}, {"difficulty": {"$exists": False}}]}
        )
